...
function anthropicImageSource(dataUrl: string): { type: "base64"; media_type: string; data: string } {
...
export class MiMoVisionClient {
  ...
  async inspect(input: InspectImageInput, image: PreparedImage): Promise<VisualObservation> {
    ...
    if (!response.ok) {
      throw new Error(`MiMo vision request failed (${response.status}): ${await errorBody(response)}`);
    }

    const payload = (await response.json()) as MiMoResponse;
    const modelResult = VisionModelResultSchema.parse(parseJsonText(contentAsText(payload.content)));
