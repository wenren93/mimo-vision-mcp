...
import { ImportImageInputSchema, InspectImageInputSchema, VisualObservationSchema } from "./schemas.js";
...
function createVisionServer(): McpServer {
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        const prepared = await assets.prepare(input.assetId, input.region, input.resolution ?? "auto");
