...
export class AssetStore {
  ...
  async prepare(
    ...
    const width = Math.max(
      1,
      Math.min(metadata.width - left, Math.round(analyzedRegion.width * metadata.width)),
    );
    const height = Math.max(
      1,
      Math.min(metadata.height - top, Math.round(analyzedRegion.height * metadata.height)),
    );
